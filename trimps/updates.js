...
function tooltip(
    ...
    if (what == "Empowerments of Nature") {
        ...
        if (game.global.challengeActive !== "Eradicated") {
            tooltipText +=
                ", at which point you'll be able to fight a " + getEmpowerment(null, true) + " enemy to earn";
        ...
        if (game.global.firing && what != "Amalgamator") {
            ...
            var aAn =
                firstChar == "A" || firstChar == "E" || firstChar == "I" || firstChar == "O" || firstChar == "U"
                    ...
                    : " a ";
            tooltipText =
                "Fire " + aAn + " " + what + ". Refunds no resources, but frees up some workspace for your Trimps.";
            costText = "";
        } else {
    ...
    if (isItIn == "equipment") {
        ...
        if (what == "Shield") {
            var blockPerShield =
                ...
                equip.blockCalculated * game.jobs.Trainer.owned * (game.jobs.Trainer.modifier / 100);
                        ...
                        if (itemToCheck[item].owned < price && typeof game.resources[item] !== "undefined") {
                            ...
                            percentOfTotal =
                                itemToCheck[item].owned > 0
                                    ? prettify(((price / itemToCheck[item].owned) * 100).toFixed(1))
                                    : 0;
                            percentOfTotal = "(" + percentOfTotal + "%)";
                        }
                    }
...
function resetGame(keepPortal, resetting) {
    ...
    if (keepPortal) {
        ...
        if (oldUniverse == 1) {
            magmite =
                game.global.magmite > 0 ? Math.floor(game.global.magmite * ((100 - getMagmiteDecayAmt()) / 100)) : 0;
        ...
        firstCustomExact =
            game.global.firstCustomExact != -1 ? game.global.firstCustomExact : game.global.lastCustomExact;
...
function message(messageString, type, lootIcon, extraClass, extraTag, htmlPrefix, extraStyle) {
    ...
    if (game.options.menu.timestamps.enabled) {
        messageString =
            (game.options.menu.timestamps.enabled == 1 ? getCurrentTime() : updatePortalTimer(true)) +
            " " +
            messageString;
    }
    if (!htmlPrefix) {
...
function updatePs(jobObj, trimps, jobName) {
    ...
    if (trimps) {
        ...
        if (getPerkLevel("Motivation_II"))
            psText *= 1 + getPerkLevel("Motivation_II") * game.portal.Motivation_II.modifier;
        if (
            increase != "gems" &&
            game.permaBoneBonuses.multitasking.owned > 0 &&
            game.resources.trimps.owned >= game.resources.trimps.realMax()
...
function checkAchieve(id, evalProperty, doubleChecking, noDisplay) {
    if (
        id === "housing" &&
        !game.achievements.oneOffs.finished[game.achievements.oneOffs.names.indexOf("Realtor")] &&
        checkHousing(false, true) >= 100
    )
            ...
            for (var x = 0; x < amount; x++) {
                ...
                if (
                    ...
                    if (item == "humaneRun" || item == "mapless")
                        htmlString +=
                            achievement.evaluate() == 0 ? "Not complete, failed for this run." : "Not complete";
        ...
        for (var x = 0; x < amount; x++) {
            ...
            if (
                ...
                if (item == "humaneRun" || item == "mapless" || item == "shielded")
                    ...
                    displayColor =
                        one && !checkFeatEarnable(achievement.names[x]) ? "achieveColorRed" : "achieveColorYellow";
            } else if ((one && achievement.finished[x]) || (!one && achievement.finished > x)) {
...
function checkFeatEarnable(which) {
    var failables = {
        ...
        Grindless: function () {
            return (
                challengeActive("Watch") && !game.challenges.Watch.enteredMap && game.buildings.Nursery.purchased == 0
            );
        },
