...
export function updateEffects() {
    ...
    for (let i = 0; i < effects.size; ++i) {
        ...
        if (multiplier === 1) {
            effectsString += capitaliseFirstLetter(effect)+ "<i></i>";
        }
        else {
            effectsString += capitaliseFirstLetter(effect) + "<i></i>x" + multiplier;
        }
        if (i < effects.size - 1) {
            effectsString += "，<i></i>";
        }
...
export function uiUpdateChances(huffy, enemy, shankInfo) {
    ...
    enPoisonChanceSpan.innerHTML = enPoisonChance.join("% 至 ");
    enBleedChanceSpan.innerHTML = enBleedChance.join("% 至 ");
    enShockChanceSpan.innerHTML = enShockChance.join("% 至 ");
...
function setupTimeAffordBtn() {
    ...
    btn.addEventListener("click", () => {
        ...
        if (time.fromSave < 0) {
            fromSaveSpan.innerHTML = "现在就可以购买它！";
        }
        else {
            fromSaveSpan.innerHTML = "购买它需要：" + convertSecondsToTime(time.fromSave);
        }
