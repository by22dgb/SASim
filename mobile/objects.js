var holidayObj = {
  ...
  checkAll: function () {
    ...
    if (
      this.lastCheck != null &&
      date.getTime() - this.lastCheck.getTime() < 120000
    )
      return;
    this.lastCheck = date;
...
var tutorial = {
    ...
    switch (game.global.tutorialStep) {
      ...
      case 4:
        if (
          ...
          game.upgrades.Supershield.allowed > 0 ||
          game.global.mapsActive
        )
          game.global.tutorialStep++;
        break;
      case 5:
        if (game.global.mapsActive || game.upgrades.Supershield.allowed > 0)
          game.global.tutorialStep++;
        break;
      case 6:
        if (
          game.upgrades.Supershield.allowed > 0 &&
          game.upgrades.Dagadder.allowed > 0 &&
          game.upgrades.Bootboost.allowed > 0
        )
          game.global.tutorialStep++;
        break;
      case 7:
        if (
          game.global.world >= 7 &&
          (game.global.preMapsActive || game.upgrades.Megamace.allowed > 0)
        )
          game.global.tutorialStep++;
        break;
      case 8:
        if (
          game.upgrades.Megamace.allowed > 0 &&
          game.upgrades.Hellishmet.allowed > 0
        )
          game.global.tutorialStep++;
        break;
      case 9:
      ...
      case 10:
        if (
          ...
          (!game.mapUnlocks.TheBlock.canRunOnce && game.global.preMapsActive)
        )
          game.global.tutorialStep++;
        break;
      case 11:
  ...
  setWinSize: function () {
    ...
    var classWidth = game.global.tutorialLg
      ? "tutorialWidthLg"
      : "tutorialWidth";
    ...
    swapClass(
      "tutorialWidth",
      classWidth,
      document.getElementById("tutorialDiv")
    );
    document.getElementById("tutorialBookmarks").style.display = game.global
      .tutorialLg
      ? "inline-block"
      : "none";
    document.getElementById("tutorialInner").style.width = game.global
      .tutorialLg
      ? "79%"
      : "100%";
    this.resetScroll();
  ...
  toggleSize: function (updateOnly) {
    ...
    this.setWinSize();
    document.getElementById("tutorialSizeBtn").className = game.global
      .tutorialLg
      ? "icomoon icon-shrink"
      : "icomoon icon-expand";
  ...
  setBookmarks: function () {
    ...
    for (var x = 0; x <= game.global.tutorialStep; x++) {
      ...
      text +=
        "<div class='tutorialBookmark" +
        selected +
        "' onclick='tutorial.setViewStep(" +
        x +
        ")'>" +
        titles[x] +
        "</div>";
  ...
  getText: function () {
    ...
    switch (this.viewingStep) {
      case 0:
        ...
        text +=
          "<br/><br/>After researching Battle, press the F key or click the 'Fight' button to send Trimps to battle. You need 1 free Trimp in your town before you can send it to fight - Trap more Trimps or wait for 2 or more unemployed Trimps to breed if you run out!<br/><br/><i>You can show/hide the ADVISOR window by pressing the 'V' key, or by clicking the gold Star on the middle right after you research Battle. You can also click the <span class='icomoon icon-close' style='color: red'></span> button to close the ADVISOR window, or the <span class='icomoon icon-expand'></span> button next to it to expand this window and see a table of contents.</i>";
        goal = "Research Battle and kill 10 Bad Guys";
        break;
      case 1: //if bloodlust has been unlocked
        text =
          "I am quite pleased. With your newly found Upgrade Book, you can now Train your Trimps to fight on their own! In order to maximize breed speed, you should teach the Trimps to only go out and fight when housing is completely full.<br/><br/>";
        text +=
          "My sensors have scanned the area, and the map of your current Zone will show you any Bad Guys with special rewards:<br/><span class='glyphicon glyphicon-apple'></span><span class='glyphicon glyphicon-tree-deciduous'></span><span class='icomoon icon-cubes'></span> - Food, Wood, or Metal respectively<br/>";
        text +=
          "<span class='glyphicon glyphicon-question-sign'></span> - New Equipment<br/><span class='glyphicon glyphicon-book'></span> - An Upgrade Book (mouse over it to see Upgrade name)<br/><span class='glyphicon glyphicon-gift'></span> - Increases your max population by 5";
        text +=
          "<br/><span class='glyphicon glyphicon-user'></span> - A foreman to help automatically build things in the queue<br/><br/>You now have lots of tools at your disposal. Keep your Trimps well equipped, research your upgrades, build more housing and you'll be in Zone 2 before you know it.";
      ...
      case 6: //entered a map
        ...
        text +=
          "<br/><br/>You'll need to clear this map 3 times to earn all of its items. Keep an eye on your City and Equipment and this will be a breeze.";
      ...
      case 9: //Unlocked megamace and hellishmet
        text =
          "You're really getting the hang of this!<br/><br/>Now would be a good time to mention that you can click the grey cogwheel icon next to the maps button to customize some nifty map settings based on what you need. If you like getting 10 stacks of Map Bonus, turn on 'Repeat to 10' and 'Exit to World' and watch your Trimps automatically travel back to your current Zone after their 10th map. If you just want to get your upgrades and get out, 'Repeat for Items' is for you. Check each setting's tooltip for more information!";
        text +=
          "<br/><br/>Speaking of automation, there's another important book we could use for our automation arsenal hiding in a map up ahead. See if you can claim it for our city!";
    ...
    document.getElementById("tutorialStep").innerHTML =
      "(" + (this.viewingStep + 1) + "/" + totalSteps + ")";
    document.getElementById("tutorialBackBtn").style.display =
      this.viewingStep > 0 ? "inline-block" : "none";
    document.getElementById("tutorialNextBtn").style.display =
      this.viewingStep < game.global.tutorialStep ? "inline-block" : "none";
    if (game.global.tutorialLg) this.setBookmarks();
  },
};
...
var alchObj = {
  ...
  load: function () {
    if (game.global.potionData != null)
      this.potionsOwned = game.global.potionData;
    else {
    ...
    if (game.global.potionAuto != null)
      this.potionAuto = game.global.potionAuto;
    ...
    this.tab.style.display =
      game.global.alchemyUnlocked || game.global.challengeActive == "Alchemy"
        ? "table-cell"
        : "none";
  },
  rewards: {
  ...
  potions: [
    {
      ...
      cost: [["Potatoes", 5, 10]],
      description:
        "Increases all Herbs found by 100% (compounding). <span class='red'>Increases Enemy Attack/Health by 75% (compounding)</span>",
    ...
    },
    {
      ...
      cost: [["Mushrooms", 5, 5]],
      description:
        "Increases all Radon gained by 10% (compounding). <span class='red'>Increases Enemy Attack/Health by 30% (compounding)</span>",
      effectText: "+#% Radon",
      enemyMult: 1.3,
      effectComp: 1.1,
    },
    {
      ...
      cost: [["Seaweed", 5, 4]],
      description:
        "Increases all non-radon resources earned by 25% additively. Reduces chance to not find a Runetrinket by 1% (compounding). <span class='red'>Increases the cost of all other Potions by 50% (compounding)</span>",
      effectText: "+#% res",
      effect: 0.25,
    },

    {
      ...
      cost: [["Firebloom", 5, 4]],
      description:
        "Nullifies 5% (compounding) of increased enemy stats from Brews while in Void Maps. <span class='red'>Increases the cost of all other Potions by 50% (compounding)</span>",
    ...
    },
    {
      ...
      cost: [["Berries", 5, 4]],
      description:
        "Increases Trimp Attack/Health by 15% additively. <span class='red'>Increases the cost of all other Potions by 50% (compounding)</span>",
      effectText: "+#% Stats",
      effect: 0.15,
    },
    {
  ...
  getPotionCost: function (potionName, getText) {
    ...
    if (potion.challenge && !potion.enemyMult) {
      for (var y = 0; y < this.potionsOwned.length; y++) {
        if (
          this.potions[y].challenge !=
          (game.global.challengeActive == "Alchemy")
        )
          continue;
        if (y != index && !this.potions[y].enemyMult)
          owned += this.potionsOwned[y]; //no cost increase for enemyMult potions
      }
    }
    for (var x = 0; x < cost.length; x++) {
      ...
      if (getText) {
        var ownedName =
          game.global.challengeActive == "Alchemy" ? "cowned" : "owned";
        var color =
          game.herbs[cost[x][0]][ownedName] < thisCost ? "red" : "green";
        costText +=
          "<span class='" +
          color +
          "'>" +
  ...
  getEnemyStats: function (map, voidMap) {
    ...
    if (voidMap) {
      baseMod *= 10;
      if (this.potionsOwned[3] > 0)
        baseMod *= this.getPotionEffect("Potion of the Void");
      return baseMod;
    }
  ...
  unlock: function () {
    if (typeof game.global.messages.Loot.alchemy === "undefined")
      game.global.messages.Loot.alchemy = true;
  },
  mapCleared: function (mapObj) {
    if (game.global.universe != 2) return;
    if (
      game.global.challengeActive != "Alchemy" &&
      !game.global.alchemyUnlocked
    )
      return;
    ...
    message(
      "You found " + prettify(amt) + " " + resource + "!",
      "Loot",
      "*leaf3",
      "alchemy",
      "alchemy"
  ...
  getDropRate: function (mapLevel) {
    ...
    if (dif < 0) return 0;
    var base =
      (2 + Math.floor(world / 10) * 5) * Math.pow(this.zoneScale, world);
  ...
  openPopup: function (updateOnly) {
    if (
      updateOnly &&
      (lastTooltipTitle != "Alchemy" || !game.global.lockTooltip)
    )
      return;
    var herbContainer = updateOnly
      ? document.getElementById("alchHerbContainer")
      : null;
    if (updateOnly && !herbContainer) updateOnly = false;
    var text = updateOnly
      ? ""
      : "<div class='alchemyTitle'>Herbs</div><div id='alchHerbContainer'>";
    var ownedName =
      game.global.challengeActive == "Alchemy" ? "cowned" : "owned";
    for (var herb in game.herbs) {
      text +=
        "<div class='alchemyPopupHerb'><span class='alchemyPopupName'>" +
        herb +
        "</span><br/>" +
        prettify(game.herbs[herb][ownedName]) +
        "</div>";
    }
    if (updateOnly) {
    ...
    for (var x = 0; x < this.potions.length; x++) {
      ...
      if ((game.global.challengeActive == "Alchemy") != potion.challenge)
        continue;
      ...
      var effectText =
        prettify(this.potionsOwned[x]) +
        " owned, " +
        potion.effectText.replace("#", prettify(effectAmt * 100));
      if (name == "Potion of Finding")
        effectText +=
          ", +" + this.getRunetrinketBonusAmt().toFixed(2) + "% RT chance";
      var btnClass = this.canAffordPotion(name)
        ? "colorSuccess"
        : "colorDisabled";
      if (updateOnly) {
        ...
        document.getElementById("alchPotionCost" + x).innerHTML =
          this.getPotionCost(name, true);
      } else
        text +=
          "<td class='alchemyPopupCraft " +
          (potion.enemyMult ? "brew" : "potion") +
          "'><div class='alchemyPopupName'>" +
          name +
          "</div><span id='alchCraftBtn" +
          x +
          "' onclick='alchObj.craftPotion(\"" +
          name +
          "\")' class='btn btn-sm " +
          btnClass +
          "' style='width: 80%; margin-left: 10%;'>Craft</span><br/><span id='alchPotionEffect" +
          x +
          "' class='alchemyPotionEffect'>" +
          effectText +
          "</span><br/><span id='alchPotionCost" +
          x +
          "' class='alchemyCraftCost'>" +
          this.getPotionCost(name, true) +
          "</span><div class='alchemyAuto'>AutoCraft up to: <input value='" +
          this.potionAuto[x] +
          "' type='number' id='potionAuto" +
          x +
          "' /></div><span class='alchemyCraftDescription'>" +
          potion.description +
          "</span></td>";
    ...
    if (game.global.challengeActive == "Alchemy") {
      text +=
        "<div class='alchemyEnemyStats'>Enemies in this dimension are enchanted, gaining +" +
        prettify(this.getEnemyStats(false, false) * 100) +
        "% enemy stats in World, +" +
        prettify(this.getEnemyStats(true, false) * 100) +
        "% in Maps, and +" +
        prettify(this.getEnemyStats(true, true) * 100) +
        "% in Void Maps. All Radon drops are increased by " +
        prettify((this.getRadonMult() - 1) * 100) +
        "%.";
      text += "</div>";
    }
    text +=
      "<div class='alchemyTitle'>Drop Rates</div><table id='alchemyDropsTable'><tbody>";
    var row1 =
      "<tr><td style='font-weight: bold; font-style: italic'>Map Level</td>";
    var row2 =
      "<tr><td style='font-weight: bold; font-style: italic'>Drop Amt</td>";
  ...
  autoCraft: function () {
    ...
    for (var x = 0; x < this.potions.length; x++) {
      ...
      if (this.canAffordPotion(this.potionNames[x]))
        this.craftPotion(this.potionNames[x]);
    }
  },
  save: function () {
    for (var x = 0; x < this.potions.length; x++) {
...
var autoBattle = {
  ...
  load: function () {
    ...
    if (!this.presets.names)
      this.presets.names = ["Preset 1", "Preset 2", "Preset 3"];
  ...
  loadPreset: function (slot) {
    ...
    for (var item in this.items) {
      ...
      if (this.settings.loadHide.enabled)
        this.items[item].hidden = this.items[item].owned ? true : false;
    }
    for (var x = 0; x < plength; x++) {
      var thisPreset = preset[x];
      if (Array.isArray(thisPreset)) {
        if (
          this.settings.loadLevel.enabled &&
          thisPreset[0] == "level" &&
          thisPreset[1] <= this.maxEnemyLevel
        ) {
          ...
          while (this.rings.mods.length < slots) {
            // Adding random mods until all the slots are filled, this is based on the lvl 15 update code. Thanks Hatterson for the fix
  ...
  settings: {
    ...
    loadLevel: {
      ...
      default: 1,
      text: [
        "Leave Enemy Level on Preset Load",
        "Set Enemy Level on Preset Load",
      ],
    },
    loadRing: {
      enabled: 1,
    ...
    Lifegiving_Gem: {
      ...
      dustIncrease: function () {
        return (
          ...
          Math.max(
            0,
            autoBattle.trimp.lifesteal - autoBattle.enemy.lifestealResist
          )
        );
      },
      startPrice: 650,
  ...
  attack: function (attacker, defender) {
    ...
    if (
      ...
      (defender.bleed.time <= 0 ||
        (this.items.Sundering_Scythe.equipped && defender.bleed.time <= 5000))
    ) {
      ...
      switch (effect) {
        ...
        case "Defensive":
          this.enemy.defense += Math.ceil(
            this.enemy.level * 0.75 * Math.pow(1.05, this.enemy.level)
          );
          break;
        case "Lifestealing":
  ...
  trimpDied: function () {
    ...
    this.resetCombat();
    if (this.sessionEnemiesKilled < this.sessionTrimpsKilled)
      swapClass(
        "abTab",
        "abTabLosing",
        document.getElementById("autoBattleTab")
      );
  ...
  getDustReward: function () {
    var amt =
      (1 + (this.enemy.level - 1) * 5) * Math.pow(1.19, this.enemy.level - 1);
  ...
  enemyDied: function () {
    ...
    if (
      this.sessionEnemiesKilled >= 100 &&
      this.sessionTrimpsKilled == 0 &&
      this.enemyLevel >= 5
    )
      giveSingleAchieve("Huffstle");
    if (
      this.enemyLevel >= 20 &&
      this.trimp.shockTime <= 0 &&
      this.trimp.bleedTime <= 0 &&
      ...
      giveSingleAchieve("Just Smack It");
    ...
    this.lootAvg.counter += this.battleTime;
    if (
      this.enemy.level == this.maxEnemyLevel &&
      this.speed == 1 &&
      this.maxEnemyLevel < 150
    ) {
      this.enemiesKilled++;
  ...
  checkLastActions: function () {
    ...
    for (var x = 0; x < this.lastActions.length; x++) {
      if (
        this.lastActions[x][2] > this.dust ||
        this.lastActions[x][6] > this.shards
      ) {
        somethinGood = true;
        break;
      }
  ...
  saveLastAction: function (type, what, cost) {
    ...
    if (
      type == "ring" ||
      ((type == "contract" || type == "cancelContract") &&
        this.items[what].dustType == "shards")
    )
      useShards = true;
    ...
    if (
      ...
      lastLastAction[5]++;
    else if (lastLastAction && lastLastAction[0] == "ring" && type == "ring")
      lastLastAction[5]++;
    else if (type == "ring")
  ...
  restoreLastAction: function () {
    ...
    if (action[0] == "upgrade") {
      ...
      if (removeMods > 0) {
        autoBattle.rings.mods.splice(
          autoBattle.rings.mods.length - removeMods,
          removeMods
        );
      }
    }
  ...
  completedVoidMap: function (zone) {
    ...
    item.owned = true;
    message(
      "You have fulfilled your Contract, and Huffy has gained access to " +
        this.cleanName(this.activeContract) +
        "!",
      "Notices"
    );
    this.activeContract = "";
  ...
  help: function () {
    var text = "<ul>";
    text +=
      "<li><b>Click on an item name to equip it. You can have 4 items equipped at the start</b>, but you can purchase 'Extra Limbs' under the Bonus button if you want some more!</li>";
    text +=
      '<li>Huffy can progress to the next level of the Spire after killing the amount of Enemies listed next to your total Dust at the top of the SA window ("Kill X"). Huffy must be fighting at your highest reached level in order to progress this counter, otherwise it will say "Farming".</li>';
    text +=
      '<li>Equipping or Unequipping an item will reset your "Session Score" at the top of the SA window.</li>';
    text +=
      "<li>You can unlock new items for Huffy to use through the 'Contracts' tab. Once you have enough Dust to start a Contract, you'll need to complete a Void Map at or above the specified Zone in U2 to complete it and unlock the item.</li>";
    text +=
      "<li>Defense is a flat damage reduction. Damage taken is equal to (Enemy Attack * Shock Modifier) - Defense.</li>";
    text +=
      "<li>Lifesteal works on Bleed damage but not Poison damage. Lifesteal is based on final damage after Shock and Defense.</li>";
    text +=
      "<li>Shock boosts Poison and Bleed damage as well as normal attack damage.</li>";
    text +=
      "<li>Shocks and Bleeds cannot be applied while the Enemy already has that status effect. Poison can.</li>";
    text +=
      "<li>Resistance against an effect works by subtracting your current resist from the Enemy's chance to cause that effect. If the Enemy has a 50% Poison Chance and you have 25% Poison Resist, the Enemy will have an effective 25% Poison Chance.</li>";
    text +=
      "<li>Enemy Affixes per level are based on a seed, but everyone has the same seed and sees the same affixes each level. Feel free to discuss strategy with others!</li>";
    text +=
      "<li>Enemies gain +1 Affix every 5 Levels until 20, then gain +1 Affix every 10 Levels</li>";
    text +=
      "<li>Huffy's minimum Attack Time is 0.5 seconds. Any reductions to Attack Time below 0.5 seconds will be converted to Slow Aura, making the Enemy attack up to 50% slower.</li>";
    text +=
      "<li>The Dust reward formula is (1 + ((EnemyLevel - 1) * 5)) * (1.19^(EnemyLevel - 1))</li>";
    ...
    tooltip(
      ...
      text,
      "autoBattle.popup()",
      "Spire Assault Help/FAQ",
      "Back to Spire Assault",
      true
    );
  ...
  getRingStatusDamage: function () {
    ...
    return (
      this.rings.level * 25 * Math.pow(1.5, Math.floor(this.rings.level / 10))
    );
  },
  getRingPoisonDamage: function () {
    if (!this.oneTimers.The_Ring.owned) return 0;
    return (
      this.rings.level * 15 * Math.pow(5, Math.floor(this.rings.level / 10))
    );
  },
  ...
  getRingUi: function () {
    var text =
      "<div class='ringContainer' style='text-align: center;'><span style='font-size: 1.2em'>The Ring - Level " +
      this.rings.level +
      "</span><br/>";
    text +=
      "+ " + prettify(this.getRingStatusDamage()) + "% Bleed/Shock Damage<br/>";
    text += "+ " + prettify(this.getRingPoisonDamage()) + " Poison Damage<br/>";
    var chance = this.getRingStatusChance();
    if (chance > 0)
      text +=
        "+ " + prettify(this.getRingStatusChance()) + "% Status Chance<br/>";
    for (var x = 0; x < this.rings.mods.length; x++) {
    ...
    if (this.rings.level < 5)
      text += "Level The Ring to 5 to earn your first customizable slot!";
    else {
      ...
      for (var x = 0; x < availableMods.length; x++) {
        options +=
          ...
          availableMods[x] +
          "'>" +
          this.ringStats[availableMods[x]].name +
          "</option>";
      }
      for (var x = 0; x < slots; x++) {
        text +=
          "<select class='autoRingSelect' id='ringSlot" +
          x +
          "' onchange='autoBattle.changeRing(this, " +
          x +
          ")' value='" +
          this.rings.mods[x] +
          "'>";
        text +=
          ...
          this.rings.mods[x] +
          "'>" +
          this.ringStats[this.rings.mods[x]].name +
          "</option>";
        text += options;
        text += "</select>";
      }
    ...
    text +=
      "</div><div class='ringContainer' style='text-align: center; padding-top: 2em;'><span class='btn btn-lg autoItemUpgrade' onclick='autoBattle.levelRing()' style='width: 90%'>Level Up! (" +
      prettify(this.getRingLevelCost()) +
      " Shards)</span><br/>";
    text +=
      "Every level increases stats of all mods on The Ring.<br/>Every 10 levels, gains a bigger boost in power!<br/>";
    if (this.rings.level < 10)
      text += "Gains status chance starting at Level 10.<br/>";
    text += "</div>";
    return text;
  },
  ringStats: {
  ...
  updateBonusPrices: function () {
    for (var bonus in this.bonuses) {
      ...
      var costColor =
        (!bonusObj.useShards && cost <= this.dust) ||
        (bonusObj.useShards && cost <= this.shards)
          ? "green"
          : "red";
    ...
    for (var oneTime in this.oneTimers) {
      ...
      var costColor =
        (!oneObj.useShards && cost <= this.dust) ||
        (oneObj.useShards && cost <= this.shards)
          ? "green"
          : "red";
  ...
  popup: function (updateOnly, statsOnly, itemsOnly, leaveMode, fromBattle) {
    ...
    if (
      (updateOnly || itemsOnly) &&
      (lastTooltipTitle != "Spire Assault" || !game.global.lockTooltip)
    )
      return;
    ...
    var pctWon =
      totalFights > 0
        ? "(" +
          Math.round((this.sessionEnemiesKilled / totalFights) * 100) +
          "%)"
        : "&nbsp;";
    ...
    if (this.maxEnemyLevel > 50) {
      shardText =
        "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + prettify(this.shards) + " Shards";
      if (this.enemyLevel > 50) {
        shardText += " (";
        var shardsPs = dustPs / 1e9;
        if (shardsPs >= 0.01 || shardsPs == 0)
          shardText += prettify(shardsPs) + " per sec)";
        else {
          shardsPs = 1 / shardsPs;
    ...
    var topText =
      ...
      shardText +
      "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" +
      (this.settings.practice.enabled == 1
        ? "<b style='color: #921707'>Practicing</b>"
        : this.enemyLevel == this.maxEnemyLevel
        ? this.maxEnemyLevel == 150
          ? "Farming (Max Level)"
          : "Kill " + (this.nextLevelCount() - this.enemiesKilled)
        : "Farming") +
      "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Enemies Killed: " +
      this.sessionEnemiesKilled +
      "&nbsp;" +
    ...
    if (!(updateOnly && statsOnly))
      buttons =
        "<div id='abLevelButtons'><span id='abDecreaseLevel' onclick='autoBattle.levelDown()' class='btn-md btn auto'>- Decrease Enemy Level -</span><span onclick='autoBattle.toggleAutoLevel()' id='abAutoLevel' class='btn btn-md auto'>Set AutoLevel On</span><span onclick='autoBattle.levelUp()' id='abIncreaseLevel' class='btn btn-md auto'>+ Increase Enemy Level +</span><span id='abHelpBtn' onclick='autoBattle.help()' class='icomoon icon-question-circle'></span><span id='abCloseBtn' onclick='cancelTooltip()' class='icomoon icon-close'></span></div>";
    text =
      "<div class='noselect'><div id='autoDust'>" +
      topText +
      "</div>" +
      buttons +
      "<div class='autoBattleTopName'>Huffy</div><div class='autoBattleTopName'>Enemy</div>";
    if (updateOnly || itemsOnly)
      document.getElementById("autoDust").innerHTML = topText;
    var trimpAttackTime = this.trimp.attackSpeed;
    var enemyAttackTime = this.enemy.attackSpeed;

    var hpPct = Math.min(
      100,
      (this.trimp.health / this.trimp.maxHealth) * 100
    ).toFixed(2);
    var EhpPct = Math.min(
      100,
    ...
    var atkPct = Math.min(
      100,
      this.trimp.lastAttack / (trimpAttackTime / 1000) / 10
    ).toFixed(2);
    var EatkPct = Math.min(
      100,
      this.enemy.lastAttack / (enemyAttackTime / 1000) / 10
    ).toFixed(2);
    if ((updateOnly && statsOnly) || itemsOnly) {
      document.getElementById("autoBattleTrimpHealthBar").style.width =
        hpPct + "%";
      document.getElementById("autoBattleTrimpAttackBar").style.width =
        atkPct + "%";
      document.getElementById("autoBattleTrimpHealth").innerHTML = prettify(
        this.trimp.health
      );
      document.getElementById("autoBattleTrimpHealthMax").innerHTML = prettify(
        this.trimp.maxHealth
      );
      document.getElementById("autoBattleEnemyHealthBar").style.width =
        EhpPct + "%";
      document.getElementById("autoBattleEnemyAttackBar").style.width =
        EatkPct + "%";
      document.getElementById("autoBattleEnemyHealth").innerHTML = prettify(
        this.enemy.health
      );
      document.getElementById("autoBattleEnemyHealthMax").innerHTML = prettify(
        this.enemy.maxHealth
      );
    } else {
      text +=
        '<div class="autoBattleBarHolder"><div style="width: ' +
        hpPct +
        '%" class="progress-bar percentColorBlue" id="autoBattleTrimpHealthBar" role="progressbar"><span class="noselect innerFightBar"><span id="autoBattleTrimpHealth">' +
        prettify(this.trimp.health) +
        '</span>/<span id="autoBattleTrimpHealthMax">' +
        prettify(this.trimp.maxHealth) +
        "</span></span></div></div>";
      text +=
        '<div class="autoBattleBarHolder"><div style="width: ' +
        EhpPct +
        '%" class="progress-bar rightBar percentColorBlue" id="autoBattleEnemyHealthBar" role="progressbar"><span class="noselect innerFightBar"><span id="autoBattleEnemyHealth">' +
        prettify(this.enemy.health) +
        '</span>/<span id="autoBattleEnemyHealthMax">' +
        prettify(this.enemy.maxHealth) +
        "</span></span></div></div>";
      text +=
        '<div class="autoBattleBarHolder"><div style="width: ' +
        atkPct +
        '%" class="progress-bar percentColorYellow" id="autoBattleTrimpAttackBar" role="progressbar"><span class="innerFightBar">&nbsp;</span></div></div>';
      text +=
        '<div class="autoBattleBarHolder"><div style="width: ' +
        EatkPct +
        '%" class="progress-bar rightBar percentColorYellow" id="autoBattleEnemyAttackBar" role="progressbar"><span class="innerFightBar">&nbsp;</span></div></div>';
    }
    var statsText = "";
    ...
    for (var x = 0; x < things.length; x++) {
      ...
      var attackText =
        prettify(baseAttack) +
        " (" +
        prettify(baseAttack * 0.8) +
        " - " +
        prettify(baseAttack * 1.2) +
        ")";
      var dustBdText =
        fighterName == "trimp"
          ? "<b>Dust Mult: </b>" + prettify(this.getDustMult() * 100) + "%"
          : "<b>Dust Value:</b> " + prettify(this.getDustReward());
      var lifestealFinal =
        Math.max(fighterObj.lifesteal - opponentObj.lifestealResist, 0) * 100;
      var lifestealDisplay;
      if (lifestealFinal <= 0 && fighterObj.lifesteal > 0)
        lifestealDisplay =
          prettify(lifestealFinal) +
          "% (" +
          prettify((fighterObj.lifesteal - opponentObj.lifestealResist) * 100) +
          "%)";
      else lifestealDisplay = prettify(lifestealFinal) + "%";

      statsText +=
        "<div class='autoStats'><div class='autoStatsBreakup'><b>Attack:</b> " +
        attackText +
        "<br/><b>Attack Time:</b> " +
        prettify(attackTime) +
        "<br/><b>Defense:</b> " +
        prettify(fighterObj.defense) +
        "<br/><b>Lifesteal:</b> " +
        lifestealDisplay;
      statsText += "</div>";
      statsText +=
        "<div class='autoStatsBreakup'>" +
        dustBdText +
        "<br/><b>Poison Resist:</b> " +
        prettify(fighterObj.poisonResist) +
        "%<br/><b>Bleed Resist:</b> " +
        prettify(fighterObj.bleedResist) +
        "%<br/><b>Shock Resist:</b> " +
        prettify(fighterObj.shockResist) +
        "%";
      statsText += "</div>";
      ...
      if (fighterObj.isTrimp)
        statsText +=
          "<b>Time:</b> " +
          formatSecondsAsClock(this.battleTime / 1000, 2) +
          "<br/>";
      else {
      ...
      if (fighterObj.slowAura > 1)
        statsText +=
          "<b>Slowing Aura:</b> " +
          prettify((fighterObj.slowAura - 1) * 100) +
          "%";
      statsText += "<br/>";
      if (fighterObj.lifestealResist)
        statsText +=
          "<b>Lifesteal Resist:</b> " +
          prettify(fighterObj.lifestealResist * 100) +
          "%";
      statsText += "</div>";
      statsText += "<br/><b>Poisoned:</b> ";
      if (fighterObj.poison.time > 0) {
        var timeText =
          opponentObj.poisonTick != 1000
            ? " every " + prettify(opponentObj.poisonTick / 1000) + " sec"
            : "every second";
        statsText +=
          prettify(fighterObj.poison.mod * fighterObj.poison.stacks) +
          " damage " +
          timeText +
          " for " +
          (fighterObj.poison.time / 1000).toFixed(1) +
          " sec. (" +
          fighterObj.poison.stacks +
          "/" +
          opponentObj.poisonStack +
          ")";
      } else statsText += "None";
      ...
      if (fighterObj.bleed.time > 0) {
        statsText +=
          "Taking " +
          prettify(fighterObj.bleed.mod * 100) +
          "% attack damage after each attack for " +
          (fighterObj.bleed.time / 1000).toFixed(1) +
          " sec.";
      } else statsText += "None";
      statsText += "<br/><b>Shock:</b> ";
      if (fighterObj.shock.time > 0) {
        var shockTime =
          fighterObj.shock.time == 9999999
            ? "<span class='icomoon icon-infinity'></span>"
            : (fighterObj.shock.time / 1000).toFixed(1);
        statsText +=
          "Taking " +
          prettify(fighterObj.shock.mod * 100) +
          "% more damage for " +
          shockTime +
          " sec.";
      } else statsText += "None";
      ...
      if (
        fighterObj.poisonChance > 0 &&
        fighterObj.poisonTime > 0 &&
        fighterObj.poisonMod > 0
      ) {
        statsText +=
          prettify(fighterObj.poisonChance - opponentObj.poisonResist) +
          "% chance to poison for " +
          prettify(fighterObj.poisonTime / 1000) +
          " sec, dealing " +
          prettify(fighterObj.poisonMod) +
          " damage per second, stacking up to " +
          fighterObj.poisonStack +
          " times.";
      } else if (
        (fighterObj.poisonChance > 0 &&
          (!ringStatusChance || ringStatusChance < fighterObj.poisonChance)) ||
        fighterObj.poisonTime > 0 ||
        (fighterObj.poisonMod > freePmod &&
          (!ringPoison || ringPoison + freePmod < fighterObj.poisonMod))
      ) {
        statsText += "<span class='abError'>";
        if (fighterObj.poisonTime <= 0)
          statsText +=
            "*You need an item that can create a Poison (Like Fists of Goo) to Poison.";
        else if (
          fighterObj.poisonChance <= 0 &&
          (!this.rings.poison.poisonChance ||
            this.rings.poison.poisonChance < fighterObj.poisonChance)
        )
          statsText += "*You need an item that grants Poison Chance to Poison.";
        else if (fighterObj.poisonMod <= 0)
          statsText += "*You need an item that grants Poison Damage to Poison.";
        statsText += "</span>";
      }
      statsText += "<br/>";
      if (
        fighterObj.bleedChance > 0 &&
        fighterObj.bleedTime > 0 &&
        fighterObj.bleedMod > 0
      ) {
        statsText +=
          prettify(fighterObj.bleedChance - opponentObj.bleedResist) +
          "% chance to bleed enemies for " +
          prettify(fighterObj.bleedTime / 1000) +
          " sec, dealing attack damage plus " +
          prettify(fighterObj.bleedMod * 100) +
          "% after each enemy attack.";
      } else if (
        (fighterObj.bleedChance > 0 &&
          (!ringStatusChance || ringStatusChance < fighterObj.bleedChance)) ||
        fighterObj.bleedTime > 0 ||
        (fighterObj.bleedMod > 0 &&
          (!ringBleedShock || ringBleedShock / 100 < fighterObj.bleedMod))
      ) {
        statsText += "<span class='abError'>";
        if (fighterObj.bleedTime <= 0)
          statsText +=
            "*You need an item that can create a Bleed (Like Rusty Dagger) to cause Bleeding.";
        else if (fighterObj.bleedChance <= 0)
          statsText +=
            "*You need an item that grants Bleed Chance to cause Bleeding.";
        else if (fighterObj.bleedMod <= 0)
          statsText +=
            "*You need an item that grants Bleed Damage to cause Bleeding.";
        statsText += "</span>";
      }
      statsText += "<br/>";

      if (
        fighterObj.shockChance > 0 &&
        fighterObj.shockTime > 0 &&
        fighterObj.shockMod > 0
      ) {
        statsText +=
          prettify(fighterObj.shockChance - opponentObj.shockResist) +
          "% chance to shock for " +
          prettify(fighterObj.shockTime / 1000) +
          " sec, causing enemies to take " +
          prettify(fighterObj.shockMod * 100) +
          "% more damage from all sources.";
      } else if (
        (fighterObj.shockChance > 0 &&
          (!ringStatusChance || ringStatusChance < fighterObj.shockChance)) ||
        fighterObj.shockTime > 0 ||
        (fighterObj.shockMod > 0 &&
          (!ringBleedShock || ringBleedShock / 100 < fighterObj.shockMod))
      ) {
        statsText += "<span class='abError'>";
        if (fighterObj.shockTime <= 0 && !this.items.Nozzled_Goggles.equipped)
          statsText +=
            "*You need an item that can create a Shock (Like Battery Stick) to Shock.";
        else if (fighterObj.shockChance <= 0)
          statsText += "*You need an item that grants Shock Chance to Shock.";
        else if (fighterObj.shockMod <= 0)
          statsText += "*You need an item that grants Shock Damage to Shock.";
        statsText += "</span>";
      }
      statsText += "<br/>";
      if (fighterName == "trimp") {
        if (this.items.Doppelganger_Signet.equipped) {
          ...
          if (this.trimp.doppDown) statsText += " Dead!";
            ...
            statsText +=
              "Health: " + prettify(this.trimp.maxHealth - this.trimp.dmgTaken);
          statsText += "&nbsp;&nbsp;&nbsp;&nbsp;";
        }
        if (this.items.Goo_Golem.equipped && this.items.Goo_Golem.active()) {
          statsText +=
            "Goo Golem: " + prettify(this.trimp.gooStored) + " Stored";
        }
      } else if (this.enemyLevel > 50) {
        if (fighterObj.explodeFreq != -1) {
          var explodeIn = (
            (fighterObj.explodeFreq - fighterObj.lastExplode) /
            1000
          ).toFixed(1);
          statsText +=
            "<b>Special:</b> Explodes for " +
            prettify(fighterObj.explodeDamage * 100) +
            "% attack damage in " +
            explodeIn +
            " sec";
          ...
          statsText +=
            "<b>Special:</b> Gains x" +
            fighterObj.berserkMod +
            " damage after every " +
            fighterObj.berserkEvery +
            " attacks. Currently x" +
            prettify(mult) +
            ".";
          ...
          statsText +=
            "<b>Special:</b> " +
            fighterObj.ethChance +
            "% chance per attack to turn ethereal, healing from all damage taken. ";
    ...
    var itemsText =
      ...
      " Equipped)";
    text +=
      "<div id='autoBattleMenuButtons'><span id='abItemsBtn' onclick='autoBattle.swapPopup(\"items\")' class='btn btn-lg autoItemUpgrade darkBorder'>" +
      itemsText +
      "</span><span onclick='autoBattle.swapPopup(\"bonuses\")' class='btn btn-lg colorNavy'>Bonuses</span><span onclick='autoBattle.swapPopup(\"contracts\")' class='btn btn-lg colorVoidy darkBorder'>Contracts</span><span onclick='autoBattle.swapPopup(\"hidden\")' class='btn btn-lg autoColorOrange darkBorder'>Hidden Items</span><span class='btn btn-lg autoItemHide darkBorder' onclick='autoBattle.toggleHideMode()'>Hide Items</span>";
    text +=
      "<span id='autoBattleRingBtn' onclick='autoBattle.swapPopup(\"rings\")' style='display: " +
      (this.oneTimers.The_Ring.owned ? "inline-block" : "none") +
      "' class='btn btn-lg autoColorTeal active darkBorder'>The Ring</span>";
    text +=
      "<span onclick='autoBattle.swapPopup(\"other\")' class='btn btn-lg autoColorGrey active darkBorder'>Misc</span></div>";
    var notesElem = document.getElementById("autoBattleNotes");
    if (!notesElem || !itemsOnly)
      text +=
        "<div id='autoBattleNotes'" +
        (this.popupMode == "items" || this.popupMode == "hidden"
          ? ""
          : " style='display: none'") +
        ">" +
        this.notes +
    ...
    var extraClass =
      this.popupMode == "other" ||
      this.popupMode == "bonuses" ||
      this.popupMode == "rings"
        ? "modeLg"
        : "modeNone";
    ...
    if (this.popupMode == "items" || this.popupMode == "hidden") {
      ...
      for (x = 0; x < itemList.length; x++) {
        ...
        var upgradeCost =
          prettify(this.upgradeCost(item)) + " " + this.getCurrencyName(item);
        line1 +=
          "<div class='autoItem autoItem" +
          equipClass +
          "' onclick='autoBattle.equip(\"" +
          item +
          "\")' onmouseover='autoBattle.hoverItem(\"" +
          item +
          "\")'>" +
          this.cleanName(item) +
          (itemObj.noUpgrade ? "" : " Lv " + itemObj.level) +
          "</div>";
        if (this.popupMode == "items") {
          if (this.hideMode)
            line2 +=
              "<div class='autoItem autoItemHide' onclick='autoBattle.hide(\"" +
              item +
              "\")'>Hide</div>";
          else if (itemObj.noUpgrade)
            line2 += "<div class='autoItem autoColorGrey'>Unupgradable</div>";
          else
            line2 +=
              "<div class='autoItem autoItemUpgrade' onclick='autoBattle.upgrade(\"" +
              item +
              "\")' onmouseover='autoBattle.hoverItem(\"" +
              item +
              "\", true)'>Upgrade (" +
          ...
          line2 +=
            "<div class='autoItem autoItemRestore' onclick='autoBattle.restore(\"" +
            item +
            "\")'>Restore</div>";
        count++;
        total++;
      ...
      if (total == 0) {
        if (this.popupMode == "hidden")
          line1 +=
            "<br/><b style='color: white; padding: 2%;'>You have no hidden items right now, but can hide items you're no longer using using the 'Hide Items' button above.</b>";
      ...
      for (var bonus in this.bonuses) {
        ...
        costText =
          ...
          prettify(cost) +
          " " +
          (bonusObj.useShards ? "Shards" : "Dust") +
          "</span>";
        text +=
          "<div id='" +
          bonus +
          "BonusBox' onclick='autoBattle.buyBonus(\"" +
          bonus +
          "\")' class='autoBonusBox'>" +
          this.cleanName(bonus) +
          "<br/>Level: " +
          bonusObj.level +
          " - " +
          costText +
          "<br/>" +
          bonusObj.description() +
          "<br/>Unlimited Purchases</div>";
      }
      var oneCount = 0;
      var ownedItems = this.countOwnedItems();
      for (var oneTime in this.oneTimers) {
        ...
        costText =
          ...
          prettify(cost) +
          " " +
          (oneObj.useShards ? "Shards" : "Dust") +
          "</span>";
        if (ownedItems < oneObj.requiredItems) {
          var need = oneObj.requiredItems - ownedItems;
          text +=
            "<div class='autoBonusBox autoOneTimerNotOwned' style='padding-top: 2%'><br/>Complete " +
            need +
            " more Contract" +
            needAnS(need) +
            " to reveal this bonus!</div>";
        } else
          text +=
            "<div onclick='autoBattle.buyOneTimer(\"" +
            oneTime +
            "\")' class='autoBonusBox autoOneTimerNotOwned'>" +
            this.cleanName(oneTime) +
            "<br/>" +
            costText +
            "<br/>" +
            oneObj.description +
      ...
      for (var oneTime in this.oneTimers) {
        ...
        if (!oneObj.owned) continue;
        text +=
          "<div class='autoBonusBox autoOneTimerOwned'>" +
          this.cleanName(oneTime) +
          "<br/><span class='green'>Owned!</span><br/>" +
          oneObj.description +
          "</div>";
      }
      text += "</div>";
      ...
      for (var x = 0; x < contracts.length; x++) {
        ...
        if (accepted)
          description =
            "You have paid the " +
            (itemObj.dustType == "shards" ? "Shards" : "Dust") +
            " and accepted this Contract.<br/>Huffy will gain access to this item as soon as you<br/><b style='font-size:1.3em'>Complete a U2 Z" +
            itemObj.zone +
            "+ Void Map</b>";
        ...
        text +=
          ...
          this.cleanName(item) +
          "</div><div class='contractDescription" +
          extraClass +
          "'>" +
          description +
          "</div>";
        if (accepted)
          text +=
            "<span onclick='autoBattle.abandonContract()' class='btn btn-lg autoItemHide'>Abandon and Refund</span>";
        else if (!this.activeContract)
          text +=
            "<span onclick='autoBattle.acceptContract(\"" +
            item +
            "\")' class='btn btn-lg colorVoidy'>Accept (" +
            prettify(this.contractPrice(item)) +
            " " +
            this.getCurrencyName(item) +
            ", Complete a Z" +
            itemObj.zone +
            " Void Map)</span>";
        else
          text +=
            "<span class='btn btn-lg autoColorGrey'>Other Contract in Progress</span>";
        text += "</div>";
      }
      if (contracts.length < 3) {
        var extraText =
          contracts.length == 0
            ? "There are no Contracts left to complete! Huffy is geared to the teeth!"
            : "&nbsp;";
        text +=
          "<div class='contractBox' style='width: " +
          (33.3 * (3 - contracts.length)).toFixed(2) +
          "%'><div class='contractTitle'>&nbsp;</div><div class='contractDescription'>" +
          extraText +
          "</div><span class='btn btn-lg colorVoidy' style='visibility: hidden'>&nbsp</span>";
      }
      ...
      for (var setting in this.settings) {
        var thisSetting = this.settings[setting];
        if (
          typeof thisSetting.hideUnless !== "undefined" &&
          !thisSetting.hideUnless()
        )
          continue;
        var className = thisSetting.enabled
          ? "autoItemEquipped"
          : "autoItemHide";
        text +=
          "<span class='btn btn-md " +
          className +
          "' onclick='autoBattle.toggleSetting(\"" +
          setting +
          "\")'>" +
          thisSetting.text[thisSetting.enabled] +
          "</span>";
      }
      text += "</div>";
      text +=
        "<div class='abMiscBox'><b style='font-size: 1.1em;'>Undo last change</b><br/>";
      var action = this.lastActions[this.lastActions.length - 1];
      if (action) {
        if (!this.confirmUndo)
          text +=
            "<span class='btn autoItemUpgrade btn-md' onclick='autoBattle.confirmUndoClicked()'>Undo</span>";
        else
          text +=
            "<b>Are you sure?!</b><br/><span class='btn autoItemUpgrade btn-md' onclick='autoBattle.restoreLastAction()'>Yes, Undo</span><span class='btn autoItemHide btn-md' onclick='autoBattle.confirmUndoClicked()'>No, Cancel</span>";
        text += "<br/>";
        if (action[0] == "ring") {
          text +=
            "Downgrade your ring by " +
            action[5] +
            " level" +
            needAnS(action[5]);
        } else {
        ...
        if (this.maxEnemyLevel >= 51)
          text += " AND SHARDS TO " + prettify(action[6]);
        text += "</b> (The amount you had the moment before the upgrade).";
        if (this.maxEnemyLevel > action[3])
          text += " Your progress will be set back to level " + action[3] + ".";
        else if (this.enemiesKilled > action[4])
          text +=
            " Your kill counter will be reduced by " +
            prettify(this.enemiesKilled - action[4]) +
            ".";
        text += "<br/>";
      } else
        text +=
          "Undoing your last 3 actions would still leave you with less currency than you have now.";
      text += "</div>";
      for (var x = 1; x <= 3; x++) {
        var pname = "p" + x;
        var preset = this.presets[pname];
        text +=
          "<div class='abMiscBox preset'><b style='font-size: 1.1em;'>" +
          this.presets.names[x - 1] +
          "</b><br/>";
        text +=
          "<span class='btn autoItemEquipped btn-md' onclick='autoBattle.savePreset(\"p" +
          x +
          "\")'>Save</span>";
        if (preset.length)
          text +=
            "<span class='btn autoItemUpgrade btn-md' onclick='autoBattle.loadPreset(\"p" +
            x +
            "\")'>Load</span>";
        text +=
          '<span class=\'btn autoColorOrange btn-md\' onclick=\'tooltip("Rename SA Preset", null, "update", ' +
          x +
          ")'>Rename</span>";
        ...
        for (var y = 0; y < preset.length; y++) {
          if (Array.isArray(preset[y])) {
            if (preset[y][0] == "level" && this.settings.loadLevel.enabled)
              text += ". Level " + preset[y][1];
...
var u2Mutations = {
  ...
  tree: {
    ...
    NovaScruff: {
      ...
      require: ["Scruffy"],
      description:
        "Scruffy gains 300% of the Zone's XP when you kill a primary Nova enemy.",
      purchased: false,
    },
    Worship: {
      pos: [-3, 11.5],
    ...
    Heirlots: {
      ...
      require: ["Nullifium"],
      description:
        "In U2 above Z200, gain a +5% chance to find your highest available Heirloom tier, and a -5% chance to find the lowest.",
      purchased: false,
    },
    Heirmazing: {
      pos: [-13, 16],
      ...
      require: ["Heirlots"],
      description:
        "In U2, you find all Heirlooms at 1 reward tier higher than normal.",
      ring: 1,
      purchased: false,
    },
    Slowva: {
      ...
      require: ["Nullifium"],
      description:
        "When attacking a Nova enemy, take only one stack of blinded every 2 attacks.",
      purchased: false,
    },
    Poppin: {
      pos: [-7, 18],
    ...
    Ragiffium: {
      ...
      ring: 1,
      description:
        "Raging Cells drop 5% of the Nullifium Value of the highest available Heirloom tier at your current Zone.",
    },
    Overkill1: {
      pos: [-3, -2],
      dn: "Overkill",
      ...
      get description() {
        return (
          "0.5% of your damage can Overkill 1 cell in U2, up to 30% of your Highest Zone reached (Overkill up to Z" +
          Math.floor(game.global.highestRadonLevelCleared * 0.3) +
          "). U1 Masteries that increase Overkill do not apply in U2."
        );
      },
      purchased: false,
    },
    Overkill2: {
      ...
      get description() {
        return (
          "You can Overkill in U2 for an additional 10% of your Highest Zone (up to 40% of Highest Zone, or Z" +
          Math.floor(game.global.highestRadonLevelCleared * 0.4) +
          ")."
        );
      },
      purchased: false,
    ...
    Overkill3: {
      ...
      get description() {
        return (
          "You can Overkill in U2 for an additional 10% of your Highest Zone (up to 50% of Highest Zone, or Z" +
          Math.floor(game.global.highestRadonLevelCleared * 0.5) +
          ")."
        );
      },
      purchased: false,
    ...
    Decompress: {
      ...
      require: ["Overkill1"],
      description:
        "Compressed enemies gain 20% less attack and health from each cell they contain.",
      purchased: false,
    },
    MaxOverkill: {
      pos: [-12, -7],
    ...
    Liq1: {
      ...
      get description() {
        return (
          "You can Liquify Zones up to 10% of your Highest Zone. (Up to Z" +
          Math.floor(game.global.highestRadonLevelCleared * 0.1) +
          ")"
        );
      },
      purchased: false,
    ...
    Smashing: {
      ...
      require: ["Liq1"],
      description:
        "+5% chance in Mutated Zones to spawn an extra Compressed Cell.",
      purchased: false,
    },
    Liq2: {
      pos: [-7, -18.5],
      ...
      get description() {
        return (
          "You can Liquify for an additional 10% of your Highest Zone (up to 20% of Highest Zone, or Z" +
          Math.floor(game.global.highestRadonLevelCleared * 0.2) +
          ")"
        );
      },
      purchased: false,
    ...
    Liq3: {
      ...
      get description() {
        ...
        var text =
          "Your Overkill Zones no longer start until after Liquification.";
        if (u2Mutations.tree.Overkill1.purchased)
          text +=
            " (With your currently purchased Mutators, Overkill up to " +
            prettify(mult * 100) +
            "% of Highest Zone, or Z" +
            Math.floor(game.global.highestRadonLevelCleared * mult) +
            ")";
        return text;
      },
      purchased: false,
    ...
    RandLoot2: {
      ...
      description:
        "Gain 50% more seconds of loot from Randomized Rows (0.75 seconds per row).",
      purchased: false,
    },
    Dust2: {
      pos: [12.65, -5.65],
    ...
    Runed: {
      ...
      require: ["RandLoot2"],
      description:
        "Increases your Runetrinket cap by 50%. Speccing out of this Mutator stops any bonus earned from Runetrinkets above your cap, but does not remove them.",
      purchased: false,
      ring: 1,
    },
    Brains: {
      ...
      get description() {
        return (
          "Increases Trimp Attack by a number based on your total stored Science. Grants +30% Attack at " +
          prettify(1e25) +
          " Science, or +300% at " +
          prettify(1e250) +
          ". At your current total of " +
          prettify(game.resources.science.owned) +
          " Science, <b>you " +
    ...
    MadMap: {
      ...
      description:
        "100% of your damage can Overkill in maps at any level. Limited to 1 cell of Overkill if above World Overkill Zone.",
      purchased: false,
      ring: 1,
    },
  },
  ...
  setAlert: function () {
    ...
    var text =
      game.options.menu.masteryTab.enabled != 0 &&
      game.global.mutatedSeeds >= this.nextCost() &&
      this.purchaseCount < Object.keys(this.tree).length
        ? "!"
        : "";
    elem.innerHTML = text;
    ...
    if (game.global.tabForMastery) updateTalentNumbers();
      ...
      document.getElementById("talentsEssenceTotal").innerHTML =
        game.options.menu.masteryTab.enabled == 2 ||
        (game.options.menu.masteryTab.enabled == 3 && text != "!")
  ...
  getMasteryAlert: function () {
    ...
    if (cost == -1) return "";
    if (game.global.essence >= cost)
      return '&nbsp;<span class="alert badge">!</span>&nbsp;&nbsp;';
  },
  toggleRespec: function (displayOnly, forceHide) {
    ...
    mutRespecBtn.innerHTML = this.respecOnPortal
      ? "Cancel Mutator Respec"
      : "Respec Mutators On Portal";
  ...
  checkRequirements: function (what, ignoreRings) {
    ...
    if (
      ...
      itemObj.ring &&
      itemObj.ring > 0 &&
      this.purchaseCount < this.rings[itemObj.ring]
    )
      return false;
  ...
  openTree: function () {
    ...
    var costText =
      Object.keys(this.tree).length > this.purchaseCount
        ? "Next Mutator Costs: " + prettify(this.nextCost())
        : "All Mutators Purchased!";
    var curTransform = this.curTransform
      ? " style='transform: " + this.curTransform + ";'"
      : "";
    var text =
      "<div style='position: relative; z-index:2;'><div style='position: absolute; top: 2%; left: 2%; display: inline-block;'><span style='font-size: 1.1em; margin-left: 0.25em;' class='btn btn-lg btn-info' onclick='u2Mutations.showNames()' id='u2MutShowNameBtn'>" +
      (game.global.showU2MutNames ? "Hide Names" : "Show Names") +
      "</span><br/><span style='font-size: 1.1em; margin-top: 0.25em;' id='swapToMasteryBtn' class='btn btn-lg btn-success' onclick='u2Mutations.swapTab(false)'>Show Masteries" +
      this.getMasteryAlert() +
      "</span></div><div style='background-color: black'>Seeds Available: " +
      prettify(game.global.mutatedSeeds) +
      "&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;" +
      costText +
      "</div><span id='mutTreeCloseBtn' class='icomoon icon-close' onclick='u2Mutations.closeTree()'></span></div><div id='mutTree'" +
      curTransform +
      ">";
    text +=
      "<div id='mutRing1' style='width: " +
      40.8 * scale +
      "px; height: " +
      33.0 * scale +
      "px; top: " +
    ...
    for (var item in this.tree) {
      ...
      text +=
        "<div onclick=\"u2Mutations.purchase('" +
        item +
        "')\" onmouseover=\"tooltip('" +
        item +
        "', 'Mutator', event)\" onmouseout=\"tooltip('hide')\" id=\"" +
        item +
        'MutatorBox" class="mutatorBox mutatorBox' +
        bgColor +
        '" style="color: ' +
        itemObj.color +
        "; width: " +
        ...
        scale * 1.5 +
        'px">';
      if (game.global.showU2MutNames)
        text += '<span class="mutTreeName">' + dn + "</span>";
      ...
      for (var x = 0; x < connect.length; x++) {
        ...
        var arrowMidpointX =
          (thisConnect[0] - distanceX / 2) * scale +
          boxScale / 2 -
          (arrowLength * scale) / 2;
        var arrowMidpointY =
          (thisConnect[1] - distanceY / 2) * scale +
          boxScale / 2 -
          (arrowLength * scale) / 2;

        text +=
          '<div class="mutArrow mutArrow' +
          color +
  ...
  rewardMutation: function (cell) {
    ...
    for (var x = 0; x < cell.u2Mutation.length; x++) {
      ...
      if (mut == "RGE") {
        ...
        if (this.tree.Ragiffium.purchased) {
          var full = getRecycleValueByRarity(
            getHeirloomRarity(game.global.world, 1, false, true)
          );
    ...
    if (game.global.desoCompletions > 0)
      reward *= game.challenges.Desolation.getTrimpMult();
    ...
    if (typeof game.global.messages.Loot.seeds === "undefined")
      game.global.messages.Loot.seeds = true;
    message(
      "You found " +
        prettify(reward) +
        " Mutated Seed" +
      ...
      "seeds",
      null,
      "background-color: " + this.getColor(cell.u2Mutation)
    );
    ...
    if (!game.global.runningChallengeSquared) {
      ...
      message(
        "You were able to take " +
          prettify(radonReward) +
          " Radon Vials from that Mutated Enemy!",
        "Loot",
        heliumIcon(true),
        "helium",
        "helium"
  ...
  addMutations: function (array) {
    ...
    for (var item in this.types) {
      if (thisTypes.indexOf(item) != -1)
        array = this.types[item].pattern(array);
    }
    return array;
  },
  ...
  types: {
    Rage: {
      ...
      stackTooltip: function () {
        ...
        return (
          "This enemy is enraged! Damage it to weaken it back down. Deals " +
          mult +
          "x attack damage, reduced by its remaining percentage of health."
        );
      },
      rewardMult: function () {
        return 1;
    ...
    Nova: {
      pattern: function (currentArray) {
        ...
        for (var y = 0; y < repeats; y++) {
          var startC = getRandomIntSeeded(
            game.global.u2MutationSeed++,
            min,
            max
          );
          var startR = getRandomIntSeeded(
            game.global.u2MutationSeed++,
            min,
            max
          );
      ...
      attacked: function () {
        if (game.global.novaMutStacks < 50) {
          if (u2Mutations.tree.Slowva.purchased)
            game.global.novaMutStacks += 0.5;
      ...
      drawStacks: function () {
        manageStacks(
          "Blinded",
          Math.ceil(game.global.novaMutStacks),
          true,
          "novaMutStacks",
          "icomoon icon-sun",
          this.stackTooltip()
      ...
      stackTooltip: function () {
        return (
          "Your Trimps are blinded by the Nova! Trimps deal " +
          prettify(this.trimpAttackMult()) +
          "x damage, and enemies deal " +
          prettify(this.enemyAttackMult()) +
          "x damage. Stacks increase when attacking Nova enemies and clear when you complete this Zone. Active in World only, can stack up to 50 times."
        );
      },
      rewardMult: function () {
    ...
    Swapper: {
      pattern: function (currentArray) {
        ...
        var repeats = this.repeats();
        for (var y = 0; y < repeats; y++) {
          var swapFrom = getRandomIntSeeded(
            game.global.u2MutationSeed++,
            0,
            froms.length
          );
          var swapTo = getRandomIntSeeded(
            game.global.u2MutationSeed++,
            0,
            tos.length
          );
    ...
    Compression: {
      pattern: function (currentArray) {
        ...
        if (
          u2Mutations.tree.Smashing.purchased ||
          u2Mutations.tree.Smashing2.purchased
        ) {
          var chance = 0;
        ...
        for (var z = 0; z < repeat; z++) {
          var start = getRandomIntSeeded(
            game.global.u2MutationSeed++,
            Math.ceil(min),
            Math.floor(max - count)
          );
          min += repeatFreq;
          max += repeatFreq;
      ...
      rewardMult: function () {
        return (
          u2Mutations.types.Rage.cellCount() /
          (this.cellCount() * this.repeats())
        );
      },
