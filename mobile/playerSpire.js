var playerSpire = {
  ...
  presetTooltip: function (slot) {
    ...
    for (var item in traps) {
      ...
      text +=
        "<span class='playerSpireTooltipTrapName' style='background-color: " +
        color +
        "'>" +
        item +
        "&nbsp;x" +
    ...
    text +=
      '<br/><br/><div class=\'spirePresetBtns\'><span onclick=\'tooltip("confirm", null, "update", "Are you sure you want to save your current Spire layout to Preset ' +
      slot +
      '? This will overwrite your currently saved layout.<br/><br/>If you want, you can also type a note to your future self below!<br/><br/><input maxlength=\\"250\\" style=\\"width: 100%\\" id=\\"spireLayoutNoteInput\\"/><br/>", "playerSpire.saveLayout(' +
      slot +
      ')", "Save to Layout ' +
      slot +
      "?\")'>Save Current Layout Here</span>";
    if (hasTraps && layout.length <= this.layout.length && !noLoad)
      text +=
        '<span onclick=\'tooltip("confirm", null, "update", "Are you sure you want to load layout ' +
        slot +
        '? This will remove all Traps and Towers currently placed in your Spire!", "playerSpire.loadLayout(' +
        slot +
        ')", "Load Layout ' +
        slot +
        "?\")'>Load This Layout</span>";
    text += "</div>";
  ...
  settingsTooltip: function () {
    ...
    text +=
      "<span class='spireOption'>Make Static:" +
      buildNiceCheckbox("spirefctStatic", "", this.settings.fctStatic) +
      "</span>";
    text +=
      "<span class='spireOption'>Trap Damage: " +
      buildNiceCheckbox("spirefctTrap", "", this.settings.fctTrap) +
      "</span>";
    if (!playerSpireTraps.Poison.locked)
      text +=
        "<span class='spireOption'>Poison Tick: " +
    ...
    text +=
      ...
      buildNiceCheckbox("spiretrapIcons", "", this.settings.trapIcons) +
      "</span>";
    text +=
      "<span class='spireOption'>Enemy Icons: " +
      buildNiceCheckbox("spireenemyIcons", "", this.settings.enemyIcons) +
      "</span>";
    text +=
      "<span class='spireOption'>Chill Effect: " +
      buildNiceCheckbox("spirechillGradient", "", this.settings.chillGradient) +
      "</span>";
    if (!playerSpireTraps.Lightning.locked)
      text +=
        "<span class='spireOption'>Shock Effect: " +
        buildNiceCheckbox("spireshockEffect", "", this.settings.shockEffect) +
        "</span>";
    text +=
      "<span class='spireOption'>Health as %: " +
      buildNiceCheckbox("spirepercentHealth", "", this.settings.percentHealth) +
      "</span>";
    text +=
      "<span class='spireOption'>Faded Enemies: " +
      buildNiceCheckbox("spireenemyFade", "", this.settings.enemyFade) +
      "</span>";
    text += "</div>";
    tooltip(
      "Spire Settings",
      ...
      "lock",
      text,
      "<span class='btn btn-info' onclick='playerSpire.saveSettings()'>Save</span><span class='btn btn-danger' onclick='cancelTooltip()'>Cancel</span>",
      "hi",
      "hi"
    );
  },
  ...
  updateRsPs: function () {
    ...
    if (elem) elem.innerHTML = prettify(this.getRsPs());
  },
  curateAvgs: function () {
  ...
  getUpgradesHtml: function () {
    ...
    for (var trapItem in playerSpireTraps) {
      ...
      html +=
        "<div onmouseover='playerSpire.upgradeTooltip(\"" +
        trapItem +
        "\", event)' onmouseout='tooltip(\"hide\")' onclick='playerSpire.buyUpgrade(\"" +
        trapItem +
  ...
  buyUpgrade: function (trapName, confirmed) {
    ...
    if (!confirmed) {
      ...
      var tipText =
        ...
        trapText +
        '? This upgrade is non-refundable!<br/><br/><i>"' +
        upgrade.description +
        '"</i><br/><br/><b>Cost: ' +
        prettify(upgrade.cost) +
        " Rs</b>";
      tooltip(
        "confirm",
        null,
        "update",
        tipText,
        "playerSpire.buyUpgrade('" + trapName + "', true)",
        "Upgrade " + trapText + "?"
      );
      return;
    }
  ...
  infoTooltip: function (what, event) {
    ...
    switch (what) {
      case "Runestones":
        ...
        tooltipText =
          "Runestones (Rs) are earned by killing Bad Guys in your Spire, and the amount of Runestones gained is directly proportional to the Max Health of the slain Bad Guy.<br/><br/>You have found " +
          prettify(curCost + upgradeCost + remaining) +
          " total Runestones.<br/><br/>" +
          prettify(upgradeCost) +
          " Runestones have been spent on Upgrades.<br/><br/>" +
          prettify(curCost) +
          " Runestones have been spent on Traps/Towers in your current layout.";
        if (game.heirlooms.Core.runestones.currentBonus > 0)
          tooltipText +=
            "<br/><br/>You are earning " +
            prettify(game.heirlooms.Core.runestones.currentBonus) +
            "% more Runestones from all sources thanks to your Spire Core!";
        break;
      case "Threat":
        tooltipText =
          "Threat rises as you kill Bad Guys in your Spire, and falls as they escape. Threat is an average of kills/escapes over some time and may not always rise immediately after a kill or fall immediately after an escape, but will always stay near what your Spire can handle.<br/><br/>More Threat means Healthier Bad Guys, which means more Runestones. Threat is also required for adding additional Floors to your Spire, increasing by 100 Threat required per Floor.<br/><br/>The highest Threat your Spire has ever reached is: <b>" +
          prettify(Math.floor(this.peakThreat)) +
          "</b><br/><br/>Displayed As: <b>Current Threat</b> / <b>Threat Required for Next Floor</b>";
        break;
      case "Enemies":
        tooltipText =
          "The amount of enemies currently allowed in your Spire.<br/><br/>Your Spire can hold 1 Bad Guy, plus an additional 2.5 Bad Guys for each Floor in your Spire (rounded up).";
        if (playerSpireTraps.Frost.level >= 7 && playerSpireTraps.Frost.owned)
          tooltipText +=
            "<br/><br/>You have an additional " +
            playerSpireTraps.Frost.owned +
            " Maximum Enemies allowed in your Spire, thanks to Frost IV.";
        tooltipText +=
          "<br/><br/>Displayed As: <b>Current Enemies in Spire</b> / <b>Maximum Enemies Allowed in Spire</b>";
        break;
      case "Spirestones":
        tooltipText =
          "Spirestones (Ss) can only be earned by recycling Spire Cores found from Spires in the World, and can be used to add Floors to your Spire or upgrade other Cores.<br/><br/>Displayed As: <b>Current Spirestones</b> / <b>Spirestones Required for Next Floor</b>";
        break;
      default:
        break;
    }
    ...
    tooltipUpdateFunction = function () {
      playerSpire.infoTooltip(what, event);
    };
  },
  ...
  drawInfo: function () {
    ...
    if (this.sealed) {
      document.getElementById("playerSpireInfoPanel").innerHTML =
        "<div style='text-align: center; font-weight: bold'>The Spire is Sealed, but you are still earning bonuses from having 11 of each Tower.<br/><br/>You can unseal the Spire if you want to, but will lose your 11th towers.<br/><br/><div onclick='playerSpire.unseal()' id='unsealSpireBtn' class='spireControlBox'>Unseal Spire</div></div><span id='playerSpireCloseBtn' class='icomoon icon-close' onclick='playerSpire.closePopup()'></span>";
      return;
    }
    if (this.smallMode) {
      this.drawSmallInfo();
    ...
    if (usingScreenReader)
      infoHtml +=
        "<h1>Spire Defense - Type Help in the input below, then press enter</h1><br/><input id='spireScreenReadInput'/><br/>";
    infoHtml +=
      "<span onmouseover='playerSpire.infoTooltip(\"Runestones\", event)' onmouseout='tooltip(\"hide\")'>Runestones: <span id='playerSpireRunestones'>" +
      prettify(this.runestones) +
      "</span><br/>Runestones per Second: <span id='RsPs'>" +
      prettify(this.getRsPs()) +
      "</span></span>";
    infoHtml +=
      "<br/><span onmouseover='playerSpire.infoTooltip(\"Enemies\", event)' onmouseout='tooltip(\"hide\")'>Enemies: <span id='playerSpireCurrentEnemies'>" +
      this.currentEnemies +
      "</span> / <span id='playerSpireMaxEnemies'>" +
      this.maxEnemies +
      "</span></span>";
    infoHtml +=
      "<br/><span onmouseover='playerSpire.infoTooltip(\"Spirestones\", event)' onmouseout='tooltip(\"hide\")' id='spirestoneBox'>" +
      this.getSpirestoneHtml() +
      "</span><br/><span onmouseover='playerSpire.infoTooltip(\"Threat\", event)' onmouseout='tooltip(\"hide\")' id='playerSpireDifficulty'>" +
    ...
    infoHtml +=
      '<div onclick=\'tooltip("confirm", null, "update", "Are you sure you want to sell all Traps and Towers? You will get back 100% of Runestones spent on them.<br/><br/>' +
      (this.paused
        ? ""
        : "<b>Protip:</b> Pause your Spire before selling your defenses if you want to avoid leaking!") +
      '", "playerSpire.resetTraps()", "Sell All?")\' class=\'spireControlBox\'>Sell All</div>';
    infoHtml +=
      "<div onclick='playerSpire.togglePause()' id='pauseSpireBtn' class='spireControlBox spirePaused" +
      (this.paused ? "Yes'>Unpause" : "'>Pause Spire") +
      "</div>";
    infoHtml +=
      "<div class='spireControlBoxDbl'><div onclick='playerSpire.presetTooltip(1)'>Layout 1</div><div onclick='playerSpire.presetTooltip(2)'>Layout 2</div></div>";
    infoHtml +=
      "<div onclick='playerSpire.selectTrap(\"shiftUp\")' onmouseout='tooltip(\"hide\")' onmouseover='playerSpire.trapTooltip(\"shiftUp\", event)' id='sellTrapBox' class='spireControlBox" +
      (this.selectedTrap == "shiftUp" ? " selected" : "") +
      "'>Shift Up</div>";
    infoHtml +=
      "<div onclick='playerSpire.selectTrap(\"shiftDown\")' onmouseout='tooltip(\"hide\")' onmouseover='playerSpire.trapTooltip(\"shiftDown\", event)' id='sellTrapBox' class='spireControlBox" +
      (this.selectedTrap == "shiftDown" ? " selected" : "") +
      "'>Shift Down</div>";
    ...
    infoHtml += "<br/><hr/>";
    infoHtml +=
      "<div onclick='playerSpire.selectTrap(\"sell\")' onmouseout='tooltip(\"hide\")' onmouseover='playerSpire.trapTooltip(\"sell\", event)' style='padding-top: 1.35vw' id='sellTrapBox' class='spireTrapBox" +
      (this.selectedTrap == "sell" ? " selected" : "") +
      "'>Sell a Trap/Tower</div>";
    var cheapestTrap = -1;
    for (var item in playerSpireTraps) {
      var trap = playerSpireTraps[item];
      if (trap.locked) continue;
      ...
      infoHtml +=
        ...
        color +
        "' onmouseout='tooltip(\"hide\")' onmouseover='playerSpire.trapTooltip(\"" +
        item +
        "\", event)' onclick='playerSpire.selectTrap(\"" +
        item +
        "\")' id='" +
        item +
        "TrapBox' class='spireTrapBox" +
        (item == this.selectedTrap ? " selected" : "") +
        "'>" +
        trapIcon +
        item +
        " " +
  ...
  drawSmallInfo: function () {
    ...
    html +=
      ...
      "</span></span><br/>";
    html +=
      "<span onmouseover='playerSpire.infoTooltip(\"Spirestones\", event)' onmouseout='tooltip(\"hide\")' id='spirestoneBox'>" +
      this.getSpirestoneHtml() +
      "</span><br/>";
    html +=
      "<span onmouseover='playerSpire.infoTooltip(\"Threat\", event)' onmouseout='tooltip(\"hide\")' id='playerSpireDifficulty'>" +
      this.getDifficultyHtml() +
      "</span><br/>";
    ...
    for (var item in playerSpireTraps) {
      ...
      html +=
        ...
        "\")' id='" +
        item +
        "TrapBox' class='spireTrapBoxSmall" +
        (item == this.selectedTrap ? " selected" : "") +
        "'>" +
        trapIcon +
        item +
  ...
  trapTooltip: function (which, event) {
    if (which == "sell") {
      tooltip(
        ...
        "customText",
        event,
        "Sell a Trap or Tower! You'll get back 100% of what you spent on the last Trap or Tower of that type.<br/><br/>(Hotkey 0 or ')"
      );
      return;
    }
    if (which == "shiftUp") {
  ...
  upgradeTooltip: function (which, event) {
    ...
    tooltipUpdateFunction = function () {
      playerSpire.upgradeTooltip(which, event);
    };
  },
  ...
  drawSpire: function () {
    ...
    for (var x = layout.length - 1; x >= 0; x--) {
      var cellWrapper =
        "<div onmouseover='playerSpire.checkDragTraps(" +
        x +
        ", event)' onmousedown='playerSpire.buildTrap(\"" +
        x +
        "\")' id='playerSpireCell" +
        x +
        "' class='noselect playerSpireCell'";
      cellWrapper += " style='";
    ...
    layoutHtml +=
      "<div id='playerSpireTutorial' style='height: " +
      tutorialHeight +
      "vh' class='niceScroll'>" +
  ...
  getSetTrapBgColor: function (cellNumber, elem) {
    ...
    if ((!trap.name || trap.name == "Fire") && this.strengthLocations.indexOf(this.getRowFromCell(cellNumber)) != -1) {
      ...
      playerSpireTraps.Frost.level >= 4 &&
      (!trap.name || trap.name == "Poison") &&
      layout.length > cellNumber + 1 &&
      playerSpire.layout[cellNumber + 1].trap.name &&
  ...
  getEnemyHtml: function (cellNumber) {
    ...
    if (cell.occupiedBy.name && this.settings.enemyIcons) {
      innerText +=
        ...
        cell.occupiedBy.name +
        " spireUniqueIcon' style='color: " +
        iconColor +
        "'></span>";
    }
  ...
  updateKills: function () {
    ...
    if (elem1) elem1.textContent = prettify(this.enemiesKilled);
    var elem2 = document.getElementById("playerSpireEscapees");
    if (elem2) elem2.textContent = prettify(this.escapees);
    var elem3 = document.getElementById("playerSpireKillPct");
    if (elem3) elem3.textContent = this.killPercent;
  },
  spawnEnemy: function (catchingUp) {
    ...
    this.layout[0].occupiedBy = {
      name: icon,
      maxHealth: health,
      health: health,
      toxicity: 0,
      threat: this.difficulty,
    };
    this.currentEnemies++;
    ...
    if (trapToSell) {
      ...
      if (trapToSell == "Strength") {
        ...
        for (var x = row * 5; x < row * 5 + 5; x++) {
          if (x !== cell) this.drawSpireCell(x);
        }
      }
  ...
  buildTrap: function (cell, forceTrap) {
    ...
    if (redrawSpire) this.drawSpire();
      ...
      if (redrawCol) this.redrawColumn(cell);
      else this.drawSpireCell(cell);
  ...
  removeLightning: function (cell) {
    var col = this.getColFromCell(cell);
    if (this.lightColumns[col] > 0) this.lightColumns[col]--;
  },
  ...
  moveEnemies: function (catchingUp) {
    ...
    if (!catchingUp) playerSpire.updateDifficultyMod();
...
var playerSpireTraps = {
  Fire: {
    ...
    upgrades: [
      ...
      },
      {
        //level 4
        description:
          "<b>Double</b> the damage of all Fire Traps, and all Fire Traps will instantly kill any enemy that has 20% or less health.", //5000
        unlockAt: 375,
        cost: 2.5e7,
      },
      {
      ...
      },
      {
        //level 7
        description:
          "<b>Dectuple</b> the damage of all Fire Traps once more, and all enemies drop 20% extra Runestones when they die on a Fire Trap.", //1m
        unlockAt: 590,
        cost: 5e11,
      },
      {
      ...
      },
      {
        //level 9
        description:
          "All Fire Traps gain <b>100x</b> damage, and all enemies drop 50% extra Runestones (+30%) when they die on a Fire Trap.", //10b
        unlockAt: 700,
        cost: 1e16,
      },
      {
  ...
  Frost: {
    ...
    upgrades: [
      ...
      },
      {
        //level 3
        description:
          "Multiply Frost Trap damage <b>by 10</b>. Chilled enemies now take 25% more damage from Fire Traps.",
        unlockAt: 275,
        cost: 5e5,
      },
      {
        //level 4
        description:
          "Multiply Frost Trap damage <b>by 5</b>. If there is a Poison Trap directly before a Frost Trap, that Poison Trap becomes 4x as effective.",
        unlockAt: 330,
        cost: 2.5e6,
      },
      {
        //level 5
        description:
          "Multiply Frost Trap damage <b>by 2</b>, and each time an enemy can't move because it is slowed, that enemy becomes worth 2% more Runestones. This effect stacks additively.",
        unlockAt: 430,
        cost: 1e8,
      },
      {
    ...
    get description() {
      ...
      if (this.level >= 5)
        desc +=
          ...
          this.rsPerSlow() +
          "% more Runestones. This effect stacks additively.";
      desc += "<br/><br/>(Hotkey 2)";
      return desc;
    },
    rsPerSlow: function () {
  ...
  Poison: {
    ...
    upgrades: [
      ...
      },
      {
        //level 3
        description:
          "If there is a Poison Trap either directly before or after another Poison Trap, both Traps gain 3x damage. If a Poison Trap has Poison Traps on both sides of itself, it gains 9x damage instead.",
        unlockAt: 400,
        cost: 5e7,
      },
      {
      ...
      },
      {
        //Level 5
        description:
          "<b>Double</b> the amount of Toxicity added when an enemy steps on any Poison Trap. In addition, if the enemy has 75% or less health remaining, the final amount of Toxicity added is multiplied by 5.",
        unlockAt: 550,
        cost: 1e11,
      },
      {
        //Level 6
        description:
          "<b>Double</b> the amount of Toxicity added when an enemy steps on any Poison Trap. In addition, if an enemy leaks, gain 10% of its total Toxicity as Runestones.",
        unlockAt: 600,
        cost: 1e12,
      },
      {
    ...
    get description() {
      var desc =
        "Adds " +
        prettify(this.totalDamage()) +
        " Toxicity when stepped on. Target will take damage equal to its total Toxicity each time it attempts to move.";
      if (this.level >= 3)
        desc +=
          "<br/><br/>If a Poison Trap is placed directly next to another Poison Trap, both Traps gain 3x damage. If a Poison Trap has other Poison Traps on both sides of itself, it gains 9x damage instead.";
      if (this.level >= 5)
        desc +=
    ...
    totalDamage: function (enemy, cell) {
      ...
      if (
        cell != null &&
        playerSpireTraps.Frost.level >= 4 &&
        playerSpire.layout.length > cell + 1 &&
        playerSpire.layout[cell + 1].trap.name &&
        playerSpire.layout[cell + 1].trap.name == "Frost"
      ) {
        dmg *= 4;
      }
  ...
  Lightning: {
    ...
    upgrades: [
      ...
      },
      {
        //Level 3
        description:
          "Lightning Trap gains <b>10x</b> damage, and Shocked now causes the target to take 4x damage and Toxicity from Traps. Towers and slows are not boosted by this extra damage.",
        unlockAt: 500,
        cost: 5e9,
      },
      {
        //Level 4
        description:
          "Lightning Trap increases the damage and effect of Fire and Poison Traps in its column by <b>10%</b>, stacking additively with other Lightning Traps in the column.",
        unlockAt: 575,
        cost: 2.5e11,
      },
      {
      ...
      },
      {
        //Level 6
        description:
          "Lightning Trap gains <b>10x</b> damage, and Shocked now causes the target to take 8x damage and Toxicity from Traps. Towers and slows are not boosted by this extra damage.",
        unlockAt: 675,
        cost: 1e15,
      },
      {
        //Level 7
        description:
          "Lightning Trap now increases the damage and effect of Fire and Poison Traps in its column by <b>20%</b>, stacking additively with other Lightning Traps in the column..",
        unlockAt: 725,
        cost: 5e16,
      },
    ],
    ...
    get description() {
      ...
      var text =
        "Deals " +
        prettify(this.totalDamage()) +
        " damage when stepped on, and afflicts the target with " +
        shockTurns +
        " stack" +
        needAnS(shockTurns) +
        " of Shocked. 1 stack of Shocked is consumed each time an enemy steps on a Trap or Tower, causing that Bad Guy to take " +
        prettify(this.shockedDamage()) +
        "x damage and " +
        prettify(this.shockedEffect()) +
        "x effect from the Trap or Tower that consumed the stack of Shocked. Shocked can boost the damage but not the effect of other Lightning Traps.";
      if (this.level >= 4)
        text +=
  ...
  Strength: {
    ...
    get description() {
      return (
        "Increases the damage of all Fire Traps on the same Floor as a Strength Tower by " +
        prettify(calcHeirloomBonus("Core", "strengthEffect", 100)) +
        "%, and when stepped on deals damage equal to the combined damage of all Fire Traps on its Floor (max of 1 Strength Tower per Floor). In addition, this Tower increases the attack of your Trimps in Maps and the World by " +
        prettify(this.getWorldBonus(true)) +
        "% (additive with other Strength Towers).<br/><br/>Your Strength Towers are currently granting a total of <b>" +
        prettify(this.getWorldBonus()) +
        "%</b> attack to your Trimps.<br/><br/>(Hotkey 5)"
      );
    },
  },
  Condenser: {
    ...
    get description() {
      return (
        "When stepped on, increases the target's Toxicity by  " +
        prettify(calcHeirloomBonus("Core", "condenserEffect", 25)) +
        "%. In addition, each Condenser Tower increases all " +
        heliumOrRadon() +
        " found by " +
        prettify(this.getWorldBonus(true)) +
        "% (additive with other Condenser Towers).<br/><br/>Your Condenser Towers are currently granting a total of <b>" +
        prettify(this.getWorldBonus()) +
        "%</b> additional " +
        heliumOrRadon() +
        " from all sources.<br/><br/>(Hotkey 6)"
      );
    },
    extraEffect: function (enemy, cell) {
  ...
  Knowledge: {
    ...
    get description() {
      return (
        "When stepped on by a Chilled enemy, Chilled becomes Frozen, slowing the target to 33% speed for 5 moves. In addition, each Knowledge Tower increases " +
        Fluffy.getName() +
        "'s Experience gain by " +
        prettify(this.getWorldBonus(true)) +
        "% (additive with other Knowledge Towers). Note that Knowledge Towers are coated with antifreeze, preventing chill effects from working until the enemy steps off of this Tower.<br/><br/>Your Knowledge Towers are currently granting a total of <b>" +
        prettify(this.getWorldBonus()) +
        "%</b> additional " +
        Fluffy.getName() +
        " Exp.<br/><br/>(Hotkey 7)"
      ...
      if (color == "black") {
        ...
        if (color == playerSpireTraps.Poison.color) {
          if (speed == -0.05) {
            ...
            if (playerSpire.settings.fctTrap) x -= 0.05;
            speed = -0.5;
            distance = 25;
          }
